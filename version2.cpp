...
struct ECCPointJ { uint64_t X, Y, Z; };

ECCPointJ toJacobian(const ECCPoint &Pnt) {
    ECCPointJ R;
    R.X = Pnt.x;
    R.Y = Pnt.y;
    R.Z = 1;
...
ECCPoint toAffine(const ECCPointJ &Pnt) {
    if (Pnt.Z == 0) { return {0, 0}; } 
    uint64_t Zinv = mod_inv(Pnt.Z, P);
    uint64_t Zinv2 = (Zinv * Zinv) % P;
    uint64_t Zinv3 = (Zinv2 * Zinv) % P;
    uint64_t x = (Pnt.X * Zinv2) % P;
    uint64_t y = (Pnt.Y * Zinv3) % P;
    return {x, y};
}


ECCPointJ ecc_double_j(const ECCPointJ &Pnt) {
    if (Pnt.Z == 0 || Pnt.Y == 0) {
        
        return {0, 1, 0}; 
    }
  
    __uint128_t Y2 = (__uint128_t)Pnt.Y * Pnt.Y % P;
    __uint128_t S = (4 * (__uint128_t)Pnt.X % P) * Y2 % P;
  
    __uint128_t X2 = (__uint128_t)Pnt.X * Pnt.X % P;
    __uint128_t Z2 = (__uint128_t)Pnt.Z * Pnt.Z % P;
    __uint128_t Z4 = (Z2 * Z2) % P;
    __uint128_t M = (3 * X2 + (__uint128_t)Acoef * Z4) % P;
    
    __uint128_t X3 = (M * M + P - 2 * S) % P;
    if ((int64_t)X3 < 0) X3 += P;
   
    __uint128_t Y4 = Y2 * Y2 % P;
    __uint128_t Y3 = (M * (S + P - (uint64_t)X3) + P - 8 * Y4) % P;
    if ((int64_t)Y3 < 0) Y3 += P;
   
    __uint128_t Z3 = (2 * (__uint128_t)Pnt.Y % P * Pnt.Z) % P;
    return {(uint64_t)X3, (uint64_t)Y3, (uint64_t)Z3};
}

ECCPointJ ecc_add_j(const ECCPointJ &Pnt, const ECCPointJ &Qnt) {
    if (Pnt.Z == 0) return Qnt;  
    if (Qnt.Z == 0) return Pnt;  
  
    __uint128_t Z1Z1 = (__uint128_t)Pnt.Z * Pnt.Z % P;
    __uint128_t Z2Z2 = (__uint128_t)Qnt.Z * Qnt.Z % P;
    __uint128_t U1 = Pnt.X * Z2Z2 % P;
    __uint128_t U2 = Qnt.X * Z1Z1 % P;
    
    __uint128_t Z1Z1Z1 = Z1Z1 * Pnt.Z % P;
    __uint128_t Z2Z2Z2 = Z2Z2 * Qnt.Z % P;
    __uint128_t S1 = Pnt.Y * Z2Z2Z2 % P;
    __uint128_t S2 = Qnt.Y * Z1Z1Z1 % P;
    if (U1 == U2) {
        if (S1 != S2) {
            ...
            return ecc_double_j(Pnt);
        }
    }
    __uint128_t H = (U2 + P - U1) % P;
    __uint128_t R = (S2 + P - S1) % P;
    __uint128_t H2 = (H * H) % P;
    __uint128_t H3 = (H2 * H) % P;
    __uint128_t U1H2 = (U1 * H2) % P;
    
    __uint128_t X3 = (R * R + P - H3 + P - 2 * U1H2) % P;
    if ((int64_t)X3 < 0) X3 += P;
    
    __uint128_t Y3 = (R * ((U1H2 + P - X3) % P) + P - S1 * H3 % P) % P;
    if ((int64_t)Y3 < 0) Y3 += P;
    
    __uint128_t Z3 = (H * Pnt.Z % P) * Qnt.Z % P;
    return {(uint64_t)X3, (uint64_t)Y3, (uint64_t)Z3};
}

ECCPoint ecc_mul(const ECCPoint &point, uint64_t k) {
    ECCPointJ R = {0, 1, 0};  
    ECCPointJ Q = toJacobian(point);
    for (int i = 0; i < 64; ++i) {
        if ((k >> i) & 1ULL) {
            if (R.Z == 0) {
                R = Q;
...
uint64_t present_encrypt_block(uint64_t block, uint64_t roundKeys[32]) {
    for (int r = 0; r < 31; ++r) {
        block ^= roundKeys[r];
        ...
        uint64_t perm = 0;
        for (int i = 0; i < 64; ++i) {
            if (tmp & (1ULL << i)) perm |= (1ULL << PBox[i]);
        }
        block = perm;
    }
    block ^= roundKeys[31];
...
uint64_t present_decrypt_block(uint64_t block, uint64_t roundKeys[32]) {
    block ^= roundKeys[31];
    for (int r = 30; r >= 0; --r) {
       
        uint64_t tmp = 0;
        for (int i = 0; i < 63; ++i) {
            if (block & (1ULL << i)) tmp |= (1ULL << ((i * 4) % 63));
        }
...
vector<uint8_t> pkcs7_unpad(const vector<uint8_t> &data) {
    ...
    if (pad == 0 || pad > 8) return data; 
    size_t n = data.size();
    for (size_t i = 0; i < pad; ++i) {
        if (data[n - 1 - i] != pad) return data; 
    }
...
int main() {
    ...
    for (size_t off = 0; off < padded.size(); off += 8) {
        ...
        uint64_t c = present_encrypt_block(blk, roundKeys);
    ...
    for (uint64_t c : cipher_blocks) {
        uint64_t m = present_decrypt_block(c, roundKeys);
        uint8_t outb[8];
...
/*
Changes made:
- ECC operations updated to use Jacobian coordinates to avoid expensive mod inverses per addition/doubling.
- ECC point-double and point-add refactored accordingly (no modular inverse inside loops).
- PRESENT key schedule computed once (round keys array) instead of per-block.
- PRESENT encryption uses a precomputed permutation lookup table (PBox) to speed up bit permutation.
- Adjusted decryption loop (inverse P-layer) accordingly.
- Minor loop adjustments (e.g., fixed bounds for inverse permutation).
- Overall reduced arithmetic by using 128-bit intermediates and static lookups.
*/
